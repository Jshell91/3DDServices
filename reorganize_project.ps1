...
# Función para copiar archivos (sin mover originales críticos)
function Copy-FileWithConfirm {
    ...
    if ($DryRun) {
        Write-Host "📄 [DRY] Copiar: $Source → $Destination" -ForegroundColor Cyan
    } else {
...
try {
    ...
    if (Test-Path "test") {
        ...
        Move-FileWithConfirm "test/modify_artwork_likes.ps1" "test/api/modify_artwork_likes.ps1"
...
if ($DryRun) {
    ...
    Write-Host "   ./reorganize_project.ps1" -ForegroundColor White
}